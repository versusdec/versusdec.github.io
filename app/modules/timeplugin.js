        ...
        let wait_load = setInterval(() => {
            ...
            if ($time.length) {
                ...
                $($time.find('input[type=number]')).on('change', (e) => {
                    +e.target.value > +e.target.max ? e.target.value = e.target.max : void 0;
                    +e.target.value < +e.target.min ? e.target.value = e.target.min : void 0;
                    update($time);
                });

                $($time.find('input[type=number]')).on('focus', (e) => {
                    e.target.select()
                });
